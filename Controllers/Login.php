...
class Login extends Controllers
{
	...
	public function loginUser()
	{
		...
		if ($_POST) {
			if (empty($_POST['txtIdentidad']) || empty($_POST['txtPassword'])) {
				$arrResponse = array('status' => false, 'msg' => 'Error de datos');
			} else {
				$strUsuario  =  strClean($_POST['txtIdentidad']);
				...
				if (empty($requestUser)) {
					...
					$arrData = $requestUser;
					if ($arrData['status'] == 1) {
						$_SESSION['idUser'] = $arrData['idempleado'];
						$_SESSION['login'] = true;
	...
	public function confirmUser(string $params){
		...
		if(empty($params)){
			...
			if(empty($arrResponse)){
				...
				$data['token'] = $strToken;
				$data['idempleado'] = $arrResponse['idempleado'];
				$data['page_functions_js'] = "function_login.js";
