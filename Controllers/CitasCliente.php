...
class CitasCliente extends Controllers
{
	...
	public function CitasCliente()
	{
		...
		$data['page_tag'] = "Citas del cliente ";
		$data['page_title'] = "Citas del cliente ";
		$data['page_name'] = "Citas del cliente";
		$data['page_functions_js'] = "function_CitasCliente.js";
		$this->views->getView($this, "citascliente", $data);
	}
		...
		if (count($arrData) > 0) {
			for ($i = 0; $i < count($arrData); $i++) {
				if ($arrData[$i]) {
					...
					//$htmlOptions .= '<input type="checkbox" value="' . $arrData[$i]['idempleado'] . '" id="vehicle1" ><label for="vehicle1"> ' . $arrData[$i]['nombre'] . ' ' . $arrData[$i]['apellido_uno'] . '</label><br>';
	...
	public function getSelectIdCliente(string $idCliente)
	{
		$htmlOptions = "";
		...
		if (count($arrData) > 0) {
			for ($i = 0; $i < count($arrData); $i++) {
				if ($arrData[$i]) {
					$htmlOptions .= '  <input type="hidden" id="listID" name="listID" value="' . $arrData[$i]['idcliente'] . '">';
				}
			}
		}
		echo $htmlOptions;
	...
	public function getCitasCliente($idCitaCliente)
	{
		if ($_SESSION['permisosMod']['r']) {
			$intCitaCliente = intval($idCitaCliente);
			$arrData = $this->model->selectCita($intCitaCliente);
			for ($i = 0; $i < count($arrData); $i++) {
				$btnView = '';
				$btnEdit = '';
				$btnDelete = '';
				...
				if ($_SESSION['permisosMod']['r']) {
					$btnView = '<button class="btn btn-info btn-sm" onClick="fntViewInfo(' . $arrData[$i]['idcita'] . ')" title="Ver"><i class="far fa-eye"></i></button>';
				}
			
				if ($_SESSION['permisosMod']['d']) {
					$btnDelete = '<button class="btn btn-danger btn-sm" onClick="fntDelInfoClicita(' . $arrData[$i]['idcita'] . ')" title="Eliminar"><i class="far fa-trash-alt"></i></button>';
				}
	...
	public function getCitaCliente($idCitaCliente)
	{
		if ($_SESSION['permisosMod']['r']) {
			$intCitaCliente = intval($idCitaCliente);
			$arrData = $this->model->selectDatosCita($intCitaCliente);
